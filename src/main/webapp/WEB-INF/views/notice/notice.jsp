...
<%-- 본문 --%>
   <div class=notice_main>  
    <div class="notice_cont">
     <table>
      ...
      <c:if test="${!empty nlist}"> <%--검색 전후 게시물 목록 있을 때 --%>
      ...
      <tr>
        ...
        <td align="center">
        <a href="notice_view?board_no=${n.board_no}&page=${page}&state=view">
        ${n.board_title}</a>
        </td>
        <td align="center">${n.board_name}</td>
        <td align="center">${fn:substring(n.board_date,0,10)}</td>
      </tr>
      </c:forEach>
       </c:if>
  ...
  <div id="bList_paging">
   ...
   <%-- 글쓰기 버튼 --%>
   <c:choose>
    <c:when test="${memid==null || !memid.equals('admin')}">
   ...
   <form action="notice" method="post" name="search">  
    ...
    <c:otherwise>
     <c:set var="mbean" value="${mdao.getMemberById(memid)}"/>
      <c:if test="${memid.equals('admin')}">
       <div class="searchArea">
	   <div class="insert_cont">
       <input type="button" value="글쓰기" onclick="location='notice_write';">
       </div>
       <%-- <form action="notice" method="post" name="search">
       <div class="notice_search">
   ...
   <%							
    ...
    MemberVO mbean = mdao.getMemberById(memid);
   %>
   ...
   %>--%>
	<%-- <div class="searchArea">
	 <div class="insert_cont">
      <input type="button" value="글쓰기" onclick="location='productQnA_write';">
     </div>

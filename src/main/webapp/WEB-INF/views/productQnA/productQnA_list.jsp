...
<%-- 본문 --%>
   <div class=notice_main>  
    <div class="notice_cont">
     <table>
      ...
      <c:if test="${!empty blist}"> <%--검색 전후 게시물 목록 있을 때 --%>
      ...
      <tr>
        ...
        <td class="product 1">
         <a href="#">
          <img id="asd" src="./images/logomain.png" style="max-width: 70px; max-height: 70px;">
         </a>
        </td>     
        <td align="center">
        <a href="productQnA_view?board_no=${b.board_no}&page=${page}&state=view">
        ${b.board_title}</a>
        </td>
  ...
  <div id="bList_paging">
   ...
   <c:if test="${(empty find_field) && (empty find_name)}"> <%--검색필드와 검색어가 없는 경우 --%>
     ...
     <c:if test="${page>1}">
      <a href="productQnA_list?page=${page-1}">[이전]</a>&nbsp;
     </c:if>
     ...
     <c:forEach var="a" begin="${startpage}" end="${endpage}" step="1">
       ...
       <c:if test="${a != page}"> <%--현재 쪽번호가 선택 안 된 경우--%>
        <a href="productQnA_list?page=${a}">[${a}]</a>&nbsp;
       </c:if>
     </c:forEach>
    ...
    <c:if test="${page < maxpage}">
     <a href="productQnA_list?page=${page+1}">[다음]</a>
    </c:if>
   </c:if>
   ...
   <c:if test="${(!empty find_field) && (!empty find_name)}"> <%--검색필드와 검색어가 있는 경우 --%>
     ...
     <c:if test="${page>1}">
      <a href="productQnA_list?page=${page-1}&find_field=${find_field}&find_name=${find_name}">[이전]</a>&nbsp;
     </c:if> 
   ...
   <%--현재 쪽번호 출력 --%>
     <c:forEach var="a" begin="${startpage}" end="${endpage}" step="1">
       ...
       <c:if test="${a != page}"> <%--현재 쪽번호가 선택 안 된 경우--%>
        <a href="productQnA_list?page=${a}&find_field=${find_field}&find_name=${find_name}">[${a}]</a>&nbsp;
       </c:if>
     </c:forEach>      
    ...
    <c:if test="${page < maxpage}">
     <a href="productQnA_list?page=${page+1}&find_field=${find_field}&find_name=${find_name}">[다음]</a>
    </c:if>
   </c:if> 
   </div>
      ...
      <tr>
       <td>
        <div class="insert_cont">
         <input class="productQnA_write" type="button" value="글쓰기" onclick="location='productQnA_write?page=${page}';">
        </div>
       
        <%-- 검색 버튼 --%>                        
        <div class="productQnA_searchBar">
        <form method="get" action="productQnA_list" >
